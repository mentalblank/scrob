...
async def sync_items(
    ...
    if media_type == MediaType.episode:
        ...
        # Also pre-load every existing row carrying one of this batch's TMDB
        # episode ids, whatever show it sits under: rows orphaned by a webhook
        # (show_id=None), and rows filed under a different show entirely. TVDB
        # keeps a continuation as extra seasons of the parent series while TMDB
        # splits it into its own show, so the server hands us the parent while
        # the episode is already stored under the split-off show — matching on
        # show_id alone recreated every one of those.
        ...
        for item in items:
            ...
            if tid:
                ep_tmdb_ids.add(tid)
        if ep_tmdb_ids:
            elsewhere = await _select_in_chunks(
                db,
            ...
            for m in elsewhere:
                media_by_tmdb.setdefault((m.tmdb_id, m.media_type), m)
    else:
...
def _group_needs_mapping(group: dict) -> bool:
    """Whether merging a group would move the episode to another position.

    The mapping only exists so TVDB browsing survives losing the TVDB-numbered
    row. Duplicates sitting at the same position — the same episode imported
    twice — take nothing with them, so requiring a mapping there just leaves
    them in place for shows neither catalogue can reconcile.
    """
    keeper = (group["keep"]["season"], group["keep"]["episode"])
    return any((m["season"], m["episode"]) != keeper for m in group["merge"])
...
async def duplicate_episodes_report(
    ...
    unmapped = sorted({
        ...
        for g in groups
        if _group_needs_mapping(g) and g.get("series_tmdb_id") not in mapped_series
    })
...
async def run_merge_duplicate_episodes(user_id: int, job_id: int | None = None):
    ...
    async with async_session() as db:
        ...
        try:
            ...
            show_ids = sorted({g["show_id"] for g in groups if _group_needs_mapping(g)})
            ...
            groups = [
                g for g in groups
                if not _group_needs_mapping(g) or g["show_id"] in mapped_shows
            ]
        ...
        try:
            ...
            for processed, show in enumerate(shows, start=1):
                ...
                try:
                    ...
                    if tvdb_api_key and show.tvdb_id:
                        poster, backdrop = await _tvdb_series_artwork(
